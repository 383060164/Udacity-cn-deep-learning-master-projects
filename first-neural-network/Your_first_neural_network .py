...
class NeuralNetwork(object):
    ...
    def train(self, features, targets):
        ...
        # Convert to 2D for both X (m*features)and Y (m,n2)  
        X=np.array(features,ndmin=2).reshape(n_records,-1)
        Y=np.array(targets,ndmin=2).reshape(n_records,-1) 
        #print(n_records)
        #print("The X shape is"+str(X.shape))
        #print("The Y shape is"+str(Y.shape))
        ####Forward propagation ####
        #layer1
        hidden_inputs = np.dot(X,self.weights_input_to_hidden) # signals into hidden layer Z1 size(m,n1)
        hidden_outputs = self.activation_function(hidden_inputs) # signals from hidden layer A1
        #layer 2 i.e output layer
        final_inputs=np.dot(hidden_outputs,self.weights_hidden_to_output) #Z2 size (m,n2)
        final_outputs=final_inputs #A2 i.e y_hat size(m,n2)
        ##### Backward propagation ###
        #output layer
        output_errors=Y-final_outputs #dA2
        output_grad=output_errors  #because y=x dZ2 size(m,n2)
        #hidden layer
        hidden_errors=np.dot(output_grad,self.weights_hidden_to_output.T) #dA1 size (m,n1)
        hidden_grad=hidden_errors*hidden_outputs*(1-hidden_outputs)  #dZ1 size(m,n1)
        
        #calculate dW
        dW2=np.dot(hidden_outputs.T,output_grad)  #dW2 size(n1,n2)
        dW1=np.dot(X.T,hidden_grad)  #dW1 size(features,n1)
        #update parameters
        self.weights_hidden_to_output += self.lr *dW2/n_records #W2 size(n1,n2)
        self.weights_input_to_hidden += self.lr * dW1/n_records #W1 szie (features,n1)
    def run(self, features):
        ...
        '''
        n_records=features.shape[0] #the number of features ??????? no its number of examples
        X=np.array(features,ndmin=2).reshape(n_records,-1)  #size (m,features)
        #### Implement the forward pass here ####
        # Hidden layer
        hidden_inputs = np.dot(X,self.weights_input_to_hidden) # signals into hidden layer Z1 size(m,n1)
        hidden_outputs = self.activation_function(hidden_inputs) # signals from hidden layer A1 size(m,n1)
